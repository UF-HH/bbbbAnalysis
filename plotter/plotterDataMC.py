...
# for h in hSigs:
for hname in sigToPlot:
		for h in sigToPlot[hname]:
				shc.addHisto (hSigs[h], hname, 'sig', (titles[hname] if hname in titles else hSigs[h].GetName()))
...
if args.dosig :
	...
	shc.setListToPlot(sigToPlot.keys(), 'sig')
...
#Define blinded table
blindedtab=False
if len(myselreg)>=4:
	if myselreg[0]=="Btag4" and myselreg[2]=="SR" and myselreg[3]=="A": blindedtab=True
#Print table	
print blindedtab	
if args.tab: shc.printTable(floatFormat='.6', printMCstat=True,blinded=blindedtab)
