...
def calculatecoeffients(sample_list,couplingstoscan):
    ...
    with open('config/vbfhh4bforcvscan.txt', 'w') as the_file:
    ...
    with open('config/vbfhh4bcvline.txt', 'w') as the_file:
     for j in range(0, len(couplingstoscan)):
         ###### target values
         couplings = couplingstoscan[j]
         t_CV  = couplings[0]
         ...
         for k in range(0, len(factors) ):
            if abs(factors[k]) == 0: factors[k]=0 
            if abs(factors[k]) < 0.001: factors[k]=0 
         print ">>> XS = %f, target (CV,C2V,kl) = (%.2f,%.1f, %.1f)" %(xsec, t_CV, t_C2V, t_kl )  
         the_file.write('%.2f %f\n'%(t_CV,xsec/(1)))
...
couplingstoscan_list = [ [-3.0 + (x*0.05),1,1] for x in range(0,121)]     
