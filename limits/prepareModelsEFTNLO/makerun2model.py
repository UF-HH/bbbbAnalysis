...
for sig in signals:
    ...
    for categ in categories:
        ...
        outc=""
        for dat in datasets:
            cstr+="%s_%i=../physicsmodel%i/datacard%s_%s_%s.txt "%(categ,dat,dat,dat,categ,signame)
        cstr+="> %s_%s_%s.txt"%(card_name,categ,signame)
        os.system('%s &'%cstr)
    ...
    for comb in combinations:
        ...
        for k in  range(0,len(comb)):
          for dat in datasets:
            combstr+="%s_%i=../physicsmodel%i/datacard%s_%s_%s.txt "%(comb[k],dat,dat,dat,comb[k],signame)
          outcomb+="%s"%comb[k]
...
if datas[0] != 'data_obs':
    for card in datacards: os.system('text2workspace.py %s -D %s'%(card, datas[0]))
else:
    for card in datacards: os.system('text2workspace.py %s'%card)