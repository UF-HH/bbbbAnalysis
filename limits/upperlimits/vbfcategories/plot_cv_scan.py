...
##Dataset properties
if dataid == '1':
  folder   = 'cv_limits_2016'
  outname  = 'cv_scan_2016'
  datalumi = "2016, 35.9 fb^{-1} (13 TeV)"
elif dataid == '2':
  folder   = 'cv_limits_2017'
  outname  = 'cv_scan_2017'
  datalumi = "2017, 41.5 fb^{-1} (13 TeV)"
elif dataid == '3':
  folder   = 'cv_limits_2018'
...
lambdas = []
#Get XS predicted 
with open("../../vbflines/config/vbfhhcvline.txt") as fobj:
	for line in fobj:
		row = line.split()
...
for ipt, lam in enumerate(lambdas):
	...
	#kl = kl.replace('.', 'd')
	# print kl
	fname = folder + '/' + 'higgsCombinecv_{kl}.AsymptoticLimits.mH120.root'.format(kl=kl)
	vals  = getVals(fname)
	xs    = coupling_xs[k]
...
graph1 = ROOT.TGraph("../../vbflines/config/vbfhhcvline.txt")
graph1.SetLineColor(6)
graph1.SetLineWidth(3)
for k in range (0,graph1.GetN()): graph1.GetY()[k] *= scaleToXS

hframe = ROOT.TH1F('hframe', '', 100, -2, 2)
