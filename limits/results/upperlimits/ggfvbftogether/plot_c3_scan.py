...
parser.add_argument('--paper', dest='paper',help='Paper:  (1) paper', required = True)
...
#print coupling_xs
#Get XS predicted for vbf
with open("../../../vbflines/config/vbfhhc3line.txt") as fobj:
	for line in fobj:
		row = line.split()
		coupling_xs2.append( float(row[1])  )
#print coupling_xs2
#Get XS predicted for ggf+vbf
for k in range(0,len(coupling_xs)): coupling_xs[k] += coupling_xs2[k] 
#print coupling_xs
...
for ipt, lam in enumerate(lambdas):
	...
	vals  = getVals(fname)
	xs    = coupling_xs[k]
	obs   = vals[5][1]*xs*scaleToXS #scaleToXS*0.0 ## FIXME
	...
	if xval==1.0:
		print " "
		print "========================================================================="
		print "Unblinding HH->bbbb analysis: kl coupling in mu_ggf+mu_vbf fit"
		print "Coupling   = ",xval
		print "Obs. limit = ",obs  /(xs*scaleToXS)
		print "Exp. limit = ",exp  /(xs*scaleToXS)
		print "+2 s.d.    = ",p2s_t/(xs*scaleToXS)
		print "+1 s.d.    = ",p1s_t/(xs*scaleToXS)
		print "-1 s.d.    = ",m1s_t/(xs*scaleToXS)
		print "-2 s.d.    = ",m2s_t/(xs*scaleToXS)
		print "========================================================================="
	k=k+1
...
#for k in range (0,graph1.GetN()): graph1.GetY()[k] += coupling_xs2[k]
#for k in range (0,graph1.GetN()): graph1.GetY()[k] *= scaleToXS

graph2 = ROOT.TGraph("../../../vbflines/config/ggfhhc3linennlo.txt")
graph2.SetLineColor(6)
graph2.SetLineWidth(3)
for k in range (0,graph2.GetN()): graph2.GetY()[k] += coupling_xs2[k]
for k in range (0,graph2.GetN()): graph2.GetY()[k] *= scaleToXS

hframe = ROOT.TH1F('hframe', '', 100, -12.0, 14.0)
...
#hframe.GetYaxis().SetTitle("95% CL on #sigma_{ggF+VBF}(pp#rightarrowHH) [fb]")
hframe.GetYaxis().SetTitle("#sigma_{ggF+VBF} (pp#rightarrowHH) [fb]")
