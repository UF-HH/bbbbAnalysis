...
def calculatecoeffients(sample_list,couplingstoscan):
    ...
    with open('config/vbfhh4bforc3scan.txt', 'w') as the_file:
    ...
    with open('config/vbfhh4bc3line.txt', 'w') as the_file:
     for j in range(0, len(couplingstoscan)):
         ###### target values
         couplings = couplingstoscan[j]
         t_CV  = couplings[0]
         ...
         for k in range(0, len(factors) ):
            if abs(factors[k]) == 0: factors[k]=0 
            if abs(factors[k]) < 0.001: factors[k]=0 
         print ">>> XS = %f, target (CV,C2V,kl) = (%.1f,%.1f, %.3f)" %(xsec, t_CV, t_C2V, t_kl )  
         the_file.write('%.3f %.4f\n'%(t_kl,xsec/(1)))
...
couplingstoscan_list = [ [1,1,-28 + (x*0.5)] for x in range(0,113)]     
