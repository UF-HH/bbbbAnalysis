...
def RunGGFMVATraining(grids,parameters):
    ...
    data_sig = arrs_sig.pandas.df(all_vars+['MC_Weight'])

    #Preselection (half the background (Weight_AnaGGF%s_MVA=0) and all signal)
    data_bkg = data_bkg[ (data_bkg['Weight_AnaGGF%s_MVA'%categ]==0)]
    ...
    data_sig['train_w'] = data_sig['MC_Weight']*500 #Has 1000 expected norm for better convergence
    data_sig.drop("MC_Weight",   axis=1, inplace=True)
...
print "      *",directory
sigsamples  = ast.literal_eval(cfgparser.get("configuration","GGFMVASignals"))
