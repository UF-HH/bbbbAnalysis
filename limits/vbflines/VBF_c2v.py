...
def calculatecoeffients(sample_list,couplingstoscan):
    ...
    with open('config/vbfhh4bforc2vscan.txt', 'w') as the_file:
    ...
    with open('config/vbfhh4bc2vline.txt', 'w') as the_file:
     for j in range(0, len(couplingstoscan)):
         ###### target values
         couplings = couplingstoscan[j]
         t_CV  = couplings[0]
         ...
         for k in range(0, len(factors) ):
            if abs(factors[k]) == 0: factors[k]=0 
            if abs(factors[k]) < 0.001: factors[k]=0 
         print ">>> XS = %f, target (CV,C2V,kl) = (%.1f,%.1f, %.1f)" %(xsec, t_CV, t_C2V, t_kl )  
         the_file.write('%.1f %f \n'%(t_C2V,xsec/(1)))
...
couplingstoscan_list = [ [1,-5.0 + (x*0.1),1] for x in range(0,126)]     
