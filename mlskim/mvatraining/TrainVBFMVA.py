...
def plot_classifier_output(all_data_train,all_data_test,all_vars,classifier,label):
    ...
    plt.savefig("myplots/%s/%s_VBFMVA_score_output.png"%(label,label) )
    plt.close('all')
...
def RunGridTraining(params,X_train,Y_train,W_train,imbalancefactor,parameters):
    ...
    skf = StratifiedKFold(n_splits=3, shuffle = True, random_state = parameters[5])
...
def RunVBFMVATraining(grids,parameters):
    ...
    data_bkg['train_w'] = arrs_bkg.pandas.df("genWeight")
    data_sig['train_w'] = arrs_sig.pandas.df("genWeight")
    ...
    ##Address the fact that is an imbalancede dataset
    ...
    print '      *the signal weight sum:',(all_data_train[all_data_train.truth == 1]['train_w']).sum()
    print '      *the background weight sum:',(all_data_train[all_data_train.truth == 0]['train_w']).sum()
    ...
    for grid in grids:
        ...
        bestgrids.append([classifier,test_rocauc])
        ngrid+=1
    print '-----------------------------------------------------------------------------------------------------'
...
print "      *",directory
sigsamples  = ast.literal_eval(cfgparser.get("configuration","VBFMVASignal"))
bkgsamples  = ast.literal_eval(cfgparser.get("configuration","VBFMVABackground"))
print "    -The list of training samples:"
print "      *Signal:",sigsamples[0]
print "      *Background:",bkgsamples[0]  
variables   = ast.literal_eval(cfgparser.get("configuration","VBFMVAVariables"))
...
print "      *",randseed 
tag         = ast.literal_eval(cfgparser.get("configuration","VBFMVAtag"))
print "    -The tag:"
print "      *",tag  
...
RunVBFMVATraining(grids,trainingarguments)
